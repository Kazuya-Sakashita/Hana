...
describe('prelaunch privacy policy route', () => {
  it('documents the waitlist purpose and conservative handling', () => {
    ...
    expect(privacyPageSource).toContain('公開前検証用に確認済み')
  ...
  it('renders privacy as a quiet public trust surface instead of text-only rules', () => {
    ...
    expect(privacyPageSource).toContain("id: 'privacy-collected'")
    expect(privacyPageSource).toContain("id: 'privacy-purpose'")
    expect(privacyPageSource).toContain("id: 'privacy-stop-delete'")
    expect(privacyPageSource).toContain('id={item.id}')
    expect(privacyPageSource).toContain('scroll-mt-24')
