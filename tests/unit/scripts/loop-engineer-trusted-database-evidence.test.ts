...
describe('ISSUE-184 trusted database evidence boundary', () => {
  ...
  it('verifies exact policy, privileges, role graph, and random owner CRUD', () => {
    ...
    expect(verifier).toContain('const applicationRoutines')
    expect(verifier).toContain('const applicationTriggers')
    expect(verifier).toContain('const applicationRoutineDependencies')
    ...
    expect(adversarial).toContain('SECURITY INVOKER')
    expect(adversarial).toContain('AFTER UPDATE ON public.profiles')
    expect(adversarial).toContain('DROP TRIGGER IF EXISTS ${dormantTrigger} ON public.profiles')
