...
import {
  ...
  buildTelemetryEvidence as buildTelemetryEvidenceContract,
  ...
  createTelemetryMetricOperationSetCommitment,
  NORTH_STAR_CONTRACT,
  ...
  type TelemetryMetricAggregateReceipt,
...
const METRIC_AGGREGATE_KEY = 'synthetic-metric-aggregate-key-32-bytes-minimum'
const METRIC_AGGREGATE_KEY_VERSION = 'synthetic-aggregate-v1'
...
beforeEach(() => {
  ...
  vi.stubEnv('TELEMETRY_METRIC_AGGREGATE_KEY_VERSION', METRIC_AGGREGATE_KEY_VERSION)
  vi.stubEnv('TELEMETRY_METRIC_AGGREGATE_COMMITMENT_KEY', METRIC_AGGREGATE_KEY)
  ...
  vi.stubEnv('TELEMETRY_DEPLOYMENT_SHA', DEPLOYMENT_SHA)
})
...
function evidenceCompletenessInputs(
  ...
  return (['funnel', 'web_vital', 'api', 'ai'] as const).map((source) => {
    if (source === 'funnel') {
      const eventIds = TELEMETRY_REQUIRED_FUNNEL_OPERATIONS.map(
        (_, index) => `20000000-0000-4000-8000-${String(index + 1).padStart(12, '0')}`,
      )
      const received = TELEMETRY_REQUIRED_FUNNEL_OPERATIONS.map((operation, index) => ({
        ...envelope(eventIds[index]!, operation),
        occurred_at_utc: new Date(
          Date.parse(window_start_utc) + 12 * 60 * 60 * 1000 + index * 60 * 1000,
        )
          .toISOString()
          .replace('.000Z', 'Z'),
      }))
      const registration = authorityRegistration(source, eventIds, {
        expected_actor: null,
        window_start_utc,
        window_end_utc,
        eligible_operations: TELEMETRY_REQUIRED_FUNNEL_OPERATIONS,
      })
      return completenessInput(source, manifest(eventIds, { source }), received, {
        window_start_utc,
        window_end_utc,
        authority_registration: registration,
      })
...
function metricAggregateReceiptsFor(
  input: ReturnType<typeof evidenceInput>,
): TelemetryMetricAggregateReceipt[] {
  const funnel = input.completeness_inputs.find((item) => item.source === 'funnel')!
  const commonBoundary = {
    ...
    authority_commitment: funnel.manifest.authority_commitment,
    universe_commitment: funnel.manifest.universe_commitment,
  }
  const operationSetCommitment = createTelemetryMetricOperationSetCommitment({
    ...commonBoundary,
    commitment_key: METRIC_AGGREGATE_KEY,
  })
  const cohortCommitment = createHmac('sha256', METRIC_AGGREGATE_KEY)
    .update('synthetic-private-cohort/v1\0')
    .update(JSON.stringify(stableEvidenceValue(commonBoundary)))
    .digest('hex')
  return (['M1', 'M2', 'M3', 'M5', 'M6', 'M7', 'M8', 'M9'] as const).map((metricId) => {
    const census = input.eligible_census.metrics.find((metric) => metric.metric_id === metricId)!
    const censoring = input.censoring_status.metrics.find(
      (metric) => metric.metric_id === metricId,
    )!
    const unsigned = {
      schema_version: TELEMETRY_METRIC_AGGREGATE_RECEIPT_SCHEMA_VERSION,
      ...commonBoundary,
      generated_at_utc: new Date(input.generated_at_utc).toISOString().replace('.000Z', 'Z'),
      metric_id: metricId,
      operation_set_commitment: operationSetCommitment,
      cohort_commitment: cohortCommitment,
      database_truth_commitment: createHmac('sha256', METRIC_AGGREGATE_KEY)
        .update('synthetic-database-truth/v1\0')
        .update(metricId)
        .update(JSON.stringify(stableEvidenceValue({ census, censoring })))
        .digest('hex'),
      memory_truth_commitment: (['M2', 'M3', 'M8', 'M9'] as const).includes(
        metricId as 'M2' | 'M3' | 'M8' | 'M9',
      )
        ? createHmac('sha256', METRIC_AGGREGATE_KEY)
            .update('synthetic-memory-truth/v1\0')
            .update(metricId)
            .digest('hex')
        : null,
      ...
      receipt_key_version: METRIC_AGGREGATE_KEY_VERSION,
    }
    return {
      ...unsigned,
      receipt_commitment: createTelemetryMetricAggregateReceiptCommitment({
        receipt: unsigned,
        commitment_key: METRIC_AGGREGATE_KEY,
      }),
    }
...
function protectedEvidenceInput(input: ReturnType<typeof evidenceInput>) {
  const {
    source_sha: _sourceSha,
    generated_at_utc: _generatedAtUtc,
    eligible_census: _eligibleCensus,
    censoring_status: _censoringStatus,
    ...protectedInput
  } = input
  return {
    ...protectedInput,
    metric_aggregate_receipts: metricAggregateReceiptsFor(input),
  }
}

function resignMetricAggregateReceipt(
  receipt: TelemetryMetricAggregateReceipt,
  overrides: Partial<TelemetryMetricAggregateReceipt>,
): TelemetryMetricAggregateReceipt {
  const { receipt_commitment: _receiptCommitment, ...currentUnsigned } = receipt
  const unsigned = { ...currentUnsigned, ...overrides }
  const rebound = {
    ...unsigned,
    operation_set_commitment: createTelemetryMetricOperationSetCommitment({
      query_version: unsigned.query_version,
      source_sha: unsigned.source_sha,
      actor_key_version: unsigned.actor_key_version,
      cohort_role: unsigned.cohort_role,
      window_start_utc: unsigned.window_start_utc,
      window_end_utc: unsigned.window_end_utc,
      required_operations: unsigned.required_operations,
      authority_commitment: unsigned.authority_commitment,
      universe_commitment: unsigned.universe_commitment,
      commitment_key: METRIC_AGGREGATE_KEY,
    }),
  }
  return {
...
function buildTelemetryEvidence(input: ReturnType<typeof evidenceInput>) {
  const previousTime = Date.now()
  vi.setSystemTime(input.generated_at_utc)
  try {
    return buildTelemetryEvidenceContract(protectedEvidenceInput(input))
  } finally {
...
describe('PII-safe telemetry schema v2', () => {
  ...
  it('fixes retention, sampling and cardinality to code allowlists', () => {
    expect(TELEMETRY_QUERY_VERSION).toBe('issue-192-v1')
...
describe('status-only evidence v2', () => {
  ...
  it('derives source SHA and generation time from protected signer state', () => {
    const valid = evidenceInput()
    const protectedInput = protectedEvidenceInput(valid)
    vi.setSystemTime('2026-10-02T12:34:56.789Z')
    const evidence = buildTelemetryEvidenceContract(
      protectedEvidenceInput({
        ...valid,
        generated_at_utc: '2026-10-02T12:34:56.789Z',
      }),
    )
    expect(evidence.source_sha).toBe(DEPLOYMENT_SHA)
    expect(evidence.generated_at_utc).toBe('2026-10-02T12:34:56.789Z')
    expect(() =>
      buildTelemetryEvidenceContract({ ...protectedInput, source_sha: 'b'.repeat(40) } as never),
    ).toThrow('invalid_input')
    expect(() =>
      buildTelemetryEvidenceContract({
        ...protectedInput,
    ...
    expect(() =>
      buildTelemetryEvidenceContract({
        ...protectedInput,
        eligible_census: valid.eligible_census,
      } as never),
    ).toThrow('invalid_input')
    vi.stubEnv('TELEMETRY_DEPLOYMENT_SHA', '')
    expect(() => buildTelemetryEvidenceContract(protectedInput)).toThrow('invalid_input')
  })

  it('requires the exact five-operation funnel truth set', () => {
    const valid = evidenceInput()
    const funnel = valid.completeness_inputs.find((item) => item.source === 'funnel')!
    for (const missingOperation of TELEMETRY_REQUIRED_FUNNEL_OPERATIONS) {
      const received = funnel.received.filter(
        (event) => event.dimensions.operation !== missingOperation,
      )
      const eventIds = received.map((event) => event.event_id)
      const signedSubset = completenessInput('funnel', manifest(eventIds), received, {
        window_start_utc: valid.window_start_utc,
        window_end_utc: valid.window_end_utc,
        authority_registration: authorityRegistration('funnel', eventIds, {
          expected_actor: null,
          window_start_utc: valid.window_start_utc,
          window_end_utc: valid.window_end_utc,
          eligible_operations: TELEMETRY_REQUIRED_FUNNEL_OPERATIONS.filter(
            (operation) => operation !== missingOperation,
          ),
        }),
      })
      expect(() =>
        buildTelemetryEvidence({
          ...
          completeness_inputs: valid.completeness_inputs.map((item) =>
            item.source === 'funnel' ? signedSubset : item,
          ),
        }),
      ).toThrow('invalid_input')
    }
  ...
  it('rejects tampered or semantically invalid protected aggregate truth', () => {
    const valid = evidenceInput()
    const protectedInput = protectedEvidenceInput(valid)
    const first = protectedInput.metric_aggregate_receipts[0]!
    expect(() =>
      buildTelemetryEvidenceContract({
        ...protectedInput,
        metric_aggregate_receipts: [
          { ...first, succeeded: first.succeeded - 1 },
          ...protectedInput.metric_aggregate_receipts.slice(1),
        ],
      }),
    ).toThrow('invalid_input')

    const m2Index = protectedInput.metric_aggregate_receipts.findIndex(
      (receipt) => receipt.metric_id === 'M2',
    )
    const m2 = protectedInput.metric_aggregate_receipts[m2Index]!
    for (const invalidM2 of [
      resignMetricAggregateReceipt(m2, {
        distinct_profiles: m2.eligible + 1,
      }),
      resignMetricAggregateReceipt(m2, {
        memory_truth_commitment: null,
      }),
      resignMetricAggregateReceipt(m2, {
        generated_at_utc: '2099-01-01T00:00:00Z',
      }),
      ...
      resignMetricAggregateReceipt(m2, {
        required_operations: TELEMETRY_REQUIRED_FUNNEL_OPERATIONS.slice(1),
      }),
      resignMetricAggregateReceipt(m2, {
        required_operations: [...TELEMETRY_REQUIRED_FUNNEL_OPERATIONS, 'api_request'] as never,
      }),
      resignMetricAggregateReceipt(m2, {
        required_operations: [
          'api_request',
          ...TELEMETRY_REQUIRED_FUNNEL_OPERATIONS.slice(1),
        ] as never,
      }),
    ]) {
      expect(() =>
        buildTelemetryEvidenceContract({
          ...protectedInput,
          metric_aggregate_receipts: protectedInput.metric_aggregate_receipts.map(
            (receipt, index) => (index === m2Index ? invalidM2 : receipt),
          ),
        }),
      ).toThrow('invalid_input')
    }
