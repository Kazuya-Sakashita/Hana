# CLAUDE.md — Hana プロジェクト運用ルール

> このファイルは **Claude Code がすべての作業前に読むべきルールブック** です。
...
3. **生成物（`src/lib/api/generated/`）を直接編集しない**。必ず `npm run openapi:gen` 経由。
...
4. OpenAPI を更新（必要なら）
5. npm run openapi:lint && npm run openapi:gen
...
- [ ] `npm run openapi:lint` が通る
- [ ] `npm run openapi:gen` 実行済み・差分コミット済み
