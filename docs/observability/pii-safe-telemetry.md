...
`hana-telemetry-evidence/v7`はprotected deployment identityから導出したsource SHA、signer clockの生成時刻、UTC window、query / event schema version、保護されたactor key version、
baseline / evaluation cohort role、別key署名済みmetric aggregate receipt set / window manifest / eligible census / censoring statusのdomain-separated keyed
...
metric aggregate receiptは`TELEMETRY_METRIC_AGGREGATE_COMMITMENT_KEY`で署名し、query、deployment SHA、signer time、actor key、cohort、window、必須5 ProductEvent operation、authority / universe、private cohort、DB / Memory truth、eligible / success / censorを拘束する。evidence signerはこのkeyを代用せず、receipt検証後のcommitmentだけをartifactへ出す。raw actor、raw event、exact countは出力しない。
