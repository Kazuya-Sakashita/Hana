...
- [x] policy述語・権限・role graphの改変をexact catalogとランダムfixture CRUDで拒否し、敵対的DB改変でverifierのfail-closedを実証する
...
WITH CHECK改変、runtime直INSERT policy、推移SET ROLEの敵対的改変でもfail-closedを確認する。candidateとtrusted-controlは兄弟directoryへ分離し、
