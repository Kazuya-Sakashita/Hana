...
ISSUE-111で許可済みの5 eventだけを使う。runtimeの相関、集約、削除はISSUE-191とISSUE-185、
...
- eventからDB Memoryへの相関は、ISSUE-191で`flow_id`と`Memory.idempotencyKey`に同じUUIDを使い、
...
| M2  | required            | `photo_selected`から30分を観測したdistinct flow | 30分未満に同じUUIDで相関したDB Memory作成  | 20 Profile / flow         | ISSUE-191完了前またはevent欠測はHold | 85%以上                                            |
| M3  | required            | `ai_draft_shown`から30分を観測したdistinct flow | 30分未満に同じUUIDで相関したDB Memory作成  | 20 Profile / flow         | ISSUE-191完了前またはevent欠測はHold | 75%以上                                            |
...
| M8  | guardrail           | 月初にMemoryを持つ観測完了Profile               | 月内に`memory_viewed`が1件以上あるProfile  | 20 Profile                | ISSUE-191のcompleteness Go前はHold   | 独立baseline cohort後、評価cohort前にProductが固定 |
...
- `memory_saved` eventの欠測と利用者離脱を同一視せず、ISSUE-191のDB相関結果を使う。
- ISSUE-191がeventごとのserver truthまたはdurable ack / retryとexpected-versus-receivedを証明できない限り、
  best-effort event依存metricのcompletenessはPASSにしない。

### Account deletion during observation

- 観測開始時のeligible censusはISSUE-191の制限付きjobで固定し、actor hashやuser IDを証跡へ出さない。
...
- raw eventとactor hashはISSUE-191の最小権限aggregate jobだけが読み、ad hoc queryやPR reviewerへ開示しない。
...
- raw eventの保持・削除はADRとISSUE-191、退会時の全key version purgeはISSUE-185を正とする。
...
- ISSUE-191、ISSUE-160、ISSUE-161、ISSUE-185の各privacy / product gateがGoである。
...
- Product、Privacy、AI vendor attestation、ISSUE-191 / 160 / 185のreviewがpendingである。
...
activeの定義とevent completenessがISSUE-191で確定するまではdiagnosticである。保存数を増やすために
...
ISSUE-191、ISSUE-160、ISSUE-161、ISSUE-185、Product / Privacy reviewが未完了である。
...
- ISSUE-191: PII-safe telemetry相関・集約基盤（ISSUE-188置換）
