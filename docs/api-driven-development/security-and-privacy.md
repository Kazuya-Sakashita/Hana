...
- RLS はPhase 2。ADR-0016の`children` CRUD tracerだけrequest-scoped roleを併用し、他resourceはRoute Handler層の認可とテストで担保する。
...
| ---------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| RLS is only a `children` CRUD tracer           | multi-resource transactions need a broader role design      | ADR-0016 NO-GO gate, ownership tests, staged follow-up                                |
