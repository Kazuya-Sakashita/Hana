...
候補コード上では`candidate-pr-gate`、`candidate-openapi-validate`、`candidate-issue-registry`をsecretなしで実行する。main固定prepareはattested exact base/head commit SHAをGit Commits API応答のcommit SHAへ照合してtree SHAを取得し、そのtree SHAでGit Trees APIの完全なtreeを取得・再照合する。commit/tree SHAの混同、SHA不一致、truncated tree、不正entry、重複pathを拒否する。blobとsubmoduleのSHA・mode差分から旧pathと新pathの和集合を作るため、実行可能pathを無関係な名前へrenameしても旧pathが分類対象に残る。免除するのは明示したroot文書と`docs/`配下のMarkdownだけとし、`tsconfig.json`、`next.config.ts`、workflow、script、非Markdown文書、未知pathはDB証跡を必須にする。callerがDB change areaを過少申告してもtrusted path分類を無効化できない。
