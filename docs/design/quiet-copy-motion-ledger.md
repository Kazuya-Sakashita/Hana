...
| record         | save pending / success                   | `ページを しまっています…` / `ページを しまいました` / `できたページを ひらきます。`                           | 保存を「しまう / ひらく」比喩で keepsake に合わせる    | 達成圧、streak、派手な success animation    | `state=save_success`, timing bucket only  | ISSUE-058 / ISSUE-063 |
...
- save success は toast の短い feedback と memory detail 遷移で示し、派手な演出を足さない
