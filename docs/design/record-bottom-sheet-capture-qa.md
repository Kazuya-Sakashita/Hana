...
| パス                       | 計測開始                                      | 計測終了                                             | 今回結果                                         |
| -------------------------- | --------------------------------------------- | ---------------------------------------------------- | ------------------------------------------------ |
| core AI path               | 既存同意済みユーザーが写真 1 枚を選択した時点 | 保存完了 feedback または `/memory/{id}?saved=1` 遷移 | Synthetic stopwatch pass: 2579ms / 30s           |
| AI skip / manual save path | 写真 1 枚を選択した時点                       | 保存完了 feedback または `/memory/{id}?saved=1` 遷移 | Synthetic stopwatch pass: 1291ms / 30s           |
...
- 保存完了の終点: success toast または `/memory/{id}?saved=1` 遷移検知。
- 実行コマンド:
...
- AI 同意 copy は写真、登録した呼び名、月齢、日付、天気、parent note を送るものとして説明する。
