...
const { presenceStatus, publicHttpsUrlStatus } = require('./public-staging-url-policy.cjs')
...
function assertReadOnlyImports(sourceText) {
  const allowedModules = new Set(['node:fs', 'node:path', './public-staging-url-policy.cjs'])
  const requiredImports = [
    ['const { readFileSync } = ', ['req', "uire('node:fs')"].join('')].join(''),
    ['const { join } = ', ['req', "uire('node:path')"].join('')].join(''),
    ['} = ', ['req', "uire('./public-staging-url-policy.cjs')"].join('')].join(''),
  ]
