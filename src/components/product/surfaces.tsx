...
export function TrustSection({
  ...
  description?: React.ReactNode
  icon?: LucideIcon
  iconTone?: QuietIconTone
  className?: string
  children?: React.ReactNode
} & Omit<React.ComponentProps<'section'>, 'title'>) {
  return (
    <KeepsakeSurface className={className} {...props}>
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="meta-label">{eyebrow}</p>
          <h2 className="mt-2 break-words font-serif text-xl leading-snug">{title}</h2>
          {description ? (
            <p className="text-ink-secondary mt-3 break-words text-sm leading-narrative">
              {description}
            </p>
          ) : null}
        </div>
...
export function DataRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="border-hairline flex min-w-0 flex-col gap-1 border-t pt-3 first:border-t-0 first:pt-0">
      <span className="text-ink-tertiary text-xs">{label}</span>
      <span className="text-ink-secondary break-words text-sm leading-narrative">{value}</span>
    </div>
  )
}
