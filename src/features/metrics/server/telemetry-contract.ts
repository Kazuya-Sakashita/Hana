...
export const TELEMETRY_EVIDENCE_SCHEMA_VERSION = 'hana-telemetry-evidence/v7' as const
...
export const TELEMETRY_TARGET_DECISION_SCHEMA_VERSION = 'hana-telemetry-target-decision/v3' as const
export const TELEMETRY_BASELINE_EVIDENCE_RECEIPT_SCHEMA_VERSION =
  'hana-telemetry-baseline-evidence-receipt/v2' as const
...
export const TELEMETRY_METRIC_WINDOW_MANIFEST_SCHEMA_VERSION =
  'hana-telemetry-metric-window-manifest/v3' as const
export const TELEMETRY_ELIGIBLE_CENSUS_SCHEMA_VERSION = 'hana-telemetry-eligible-census/v2' as const
export const TELEMETRY_CENSORING_STATUS_SCHEMA_VERSION =
  'hana-telemetry-censoring-status/v2' as const
export const TELEMETRY_METRIC_AGGREGATE_RECEIPT_SCHEMA_VERSION =
  'hana-telemetry-metric-aggregate-receipt/v1' as const
export const TELEMETRY_QUERY_VERSION = 'issue-192-v1' as const
...
const COMMITMENT_DOMAINS = [
  ...
  'censoring_status',
  'completeness_scope',
  'metric_aggregate_receipts',
] as const
const TELEMETRY_EVIDENCE_DIGEST_DOMAIN = 'hana-telemetry-evidence-digest/v1\0'
const TELEMETRY_METRIC_AGGREGATE_RECEIPT_DOMAIN = 'hana-telemetry-metric-aggregate-receipt/v1\0'
const TELEMETRY_METRIC_OPERATION_SET_DOMAIN = 'hana-telemetry-metric-operation-set/v1\0'
...
export const TELEMETRY_REQUIRED_FUNNEL_OPERATIONS = [
  'record_started',
  'photo_selected',
  'ai_draft_shown',
  'memory_saved',
  'memory_viewed',
] as const satisfies readonly TelemetryOperation[]
const TELEMETRY_AGGREGATE_METRIC_IDS = ['M1', 'M2', 'M3', 'M5', 'M6', 'M7', 'M8', 'M9'] as const
...
const TELEMETRY_METRIC_POLICY_VERSION = 'immutable-metric-policy/v3' as const
const TELEMETRY_TARGET_POLICY_VERSION = 'protected-baseline-target/v3' as const
...
function hasExactRequiredFunnelOperationSet(values: readonly unknown[]): boolean {
  return (
    values.length === TELEMETRY_REQUIRED_FUNNEL_OPERATIONS.length &&
    new Set(values).size === values.length &&
    TELEMETRY_REQUIRED_FUNNEL_OPERATIONS.every((operation) => values.includes(operation))
  )
}
...
export type TelemetryMetricAggregateReceipt = {
  schema_version: typeof TELEMETRY_METRIC_AGGREGATE_RECEIPT_SCHEMA_VERSION
  query_version: typeof TELEMETRY_QUERY_VERSION
  source_sha: string
  generated_at_utc: string
  actor_key_version: string
  cohort_role: 'baseline' | 'evaluation'
  window_start_utc: string
  window_end_utc: string
  metric_id: (typeof TELEMETRY_AGGREGATE_METRIC_IDS)[number]
  required_operations: readonly (typeof TELEMETRY_REQUIRED_FUNNEL_OPERATIONS)[number][]
  operation_set_commitment: string
  cohort_commitment: string
  authority_commitment: string
  universe_commitment: string
  database_truth_commitment: string
  memory_truth_commitment: string | null
  eligible: number
...
function protectedDeploymentSha(): string | null {
  const configured = process.env.TELEMETRY_DEPLOYMENT_SHA
  return typeof configured === 'string' && SHA_PATTERN.test(configured) ? configured : null
}

function signerGeneratedAtUtc(): string {
  return new Date(Date.now()).toISOString().replace('.000Z', 'Z')
}
...
export function createTelemetryMetricOperationSetCommitment(input: {
  query_version: typeof TELEMETRY_QUERY_VERSION
  source_sha: string
  actor_key_version: string
  cohort_role: 'baseline' | 'evaluation'
  ...
  const { commitment_key: key, ...boundary } = input
  return createHmac('sha256', key)
    .update(TELEMETRY_METRIC_OPERATION_SET_DOMAIN)
    .update(JSON.stringify(stableValue(boundary)))
    .digest('hex')
}

export function createTelemetryMetricAggregateReceiptCommitment(input: {
  receipt: Omit<TelemetryMetricAggregateReceipt, 'receipt_commitment'>
  commitment_key: string
}): string {
  if (
    typeof input.commitment_key !== 'string' ||
  ...
  return createHmac('sha256', input.commitment_key)
    .update(TELEMETRY_METRIC_AGGREGATE_RECEIPT_DOMAIN)
    .update(JSON.stringify(stableValue(input.receipt)))
    .digest('hex')
}
...
function configuredCommitmentKeys(): readonly (string | undefined)[] {
  return [
    ...
    process.env.TELEMETRY_METRIC_AGGREGATE_COMMITMENT_KEY,
...
export function buildTelemetryBaselineEvidenceReceipt(input: {
  metric_id: 'M8' | 'M9'
  evidence: TelemetryEvidence
  ...
  if (
    !isRecord(input) ||
    !hasExactKeys(input, ['metric_id', 'evidence']) ||
    !verifyTelemetryEvidence(input.evidence) ||
    input.evidence.cohort_role !== 'baseline' ||
  ...
  const metric = input.evidence.metrics.find((item) => item.metric_id === input.metric_id)
  if (metric?.status !== 'HOLD' || metric.reason !== 'baseline_only') {
    throw new TelemetryContractError('invalid_input')
  }
  const unsigned = {
    ...
    window_start_utc: input.evidence.window_start_utc,
    window_end_utc: input.evidence.window_end_utc,
    generated_at_utc: input.evidence.generated_at_utc,
  ...
  return {
    ...unsigned,
    receipt_commitment: createTelemetryBaselineEvidenceReceiptCommitment({
      receipt: unsigned,
...
function validBaselineEvidenceReceipt(
  ...
  if (
    ...
    !SHA256_PATTERN.test(receipt.receipt_commitment) ||
    !verifyTelemetryEvidence(receipt.evidence) ||
    receipt.evidence.cohort_role !== 'baseline' ||
    receipt.evidence.schema_version !== receipt.evidence_schema_version ||
    receipt.evidence.query_version !== receipt.query_version ||
    receipt.evidence.actor_key_version !== receipt.actor_key_version ||
    receipt.evidence.window_start_utc !== receipt.window_start_utc ||
    receipt.evidence.window_end_utc !== receipt.window_end_utc ||
    receipt.evidence.generated_at_utc !== receipt.generated_at_utc ||
    receipt.evidence.evidence_key_version !== receipt.evidence_key_version ||
    receipt.evidence.evidence_digest !== receipt.evidence_digest ||
    !receipt.evidence.metrics.some(
      (metric) =>
        metric.metric_id === metricId &&
        metric.status === receipt.metric_status &&
        metric.reason === receipt.metric_reason,
    )
  ) {
    return false
  }
...
const METRIC_REASON_ALLOWLIST = {
  ...
  M8: [...EVIDENCE_RATE_REASONS, 'telemetry_incomplete', 'target_not_configured', 'baseline_only'],
  M9: [
    ...EVIDENCE_RATE_REASONS,
    ...FUNNEL_CORRELATION_HOLD_REASONS,
    'target_not_configured',
    'baseline_only',
  ],
...
function validMetricResult(metric: TelemetryMetricResult): boolean {
  return (
    isRecord(metric) &&
    hasExactKeys(metric, ['metric_id', 'status', 'reason']) &&
...
function validCompletenessResult(result: TelemetryCompletenessResult): boolean {
  if (
    !isRecord(result) ||
    !hasExactKeys(result, ['source', 'status', 'reason', 'duplicate', 'reorder'])
  ) {
    return false
  }
...
export type TelemetryEvidenceCompletenessResult = TelemetryCompletenessResult & {
  query_version: typeof TELEMETRY_QUERY_VERSION
  actor_key_version: string
  window_start_utc: string
  window_end_utc: string
  cohort_scope: 'all_actors' | 'expected_actor_only'
  scope_commitment: string
}

function validEvidenceCompletenessResult(
  result: unknown,
): result is TelemetryEvidenceCompletenessResult {
  if (
    !isRecord(result) ||
    !hasExactKeys(result, [
      'source',
    ...
    result.query_version !== TELEMETRY_QUERY_VERSION ||
    result.actor_key_version !== protectedActorKeyVersion() ||
    parseUtc(String(result.window_start_utc)) === null ||
    parseUtc(String(result.window_end_utc)) === null ||
    parseUtc(String(result.window_start_utc))! >= parseUtc(String(result.window_end_utc))! ||
    !['all_actors', 'expected_actor_only'].includes(String(result.cohort_scope)) ||
    typeof result.scope_commitment !== 'string' ||
    !SHA256_PATTERN.test(result.scope_commitment)
  ) {
    return false
  }
  return validCompletenessResult({
...
function validEvidenceCompletenessInput(
  value: unknown,
  windowStartUtc: string,
  windowEndUtc: string,
): value is TelemetryCompletenessInput {
  if (
    !isRecord(value) ||
    !hasExactKeys(value, [
      'source',
    ...
    value.manifest.query_version !== TELEMETRY_QUERY_VERSION ||
    !isRecord(value.authority_registration) ||
    !isRecord(value.event_universe)
  ) {
    return false
  }
  if (value.source !== 'funnel') return true
  if (
    value.authority_registration.expected_actor !== null ||
    value.authority_registration.cohort_rule !== 'all_actors' ||
    !Array.isArray(value.authority_registration.eligible_operations) ||
    !hasExactRequiredFunnelOperationSet(value.authority_registration.eligible_operations) ||
    !Array.isArray(value.event_universe.eligible_events)
  ) {
    return false
  }
  const universeOperations = value.event_universe.eligible_events.map((event) =>
    isRecord(event) ? event.operation : undefined,
  )
  return hasExactRequiredFunnelOperationSet([...new Set(universeOperations)])
}

function deriveEvidenceStatus(input: {
  completeness: readonly Pick<TelemetryCompletenessResult, 'status'>[]
  ...
  return input.metrics.some((metric) => metric.status === 'FAIL') ? 'FAIL' : 'PASS'
}

function validEvidenceCompletenessPropagation(input: {
  completeness: readonly Pick<TelemetryCompletenessResult, 'source' | 'status' | 'reason'>[]
  metrics: readonly TelemetryMetricResult[]
}): boolean {
  const funnel = input.completeness.find((item) => item.source === 'funnel')
  if (funnel?.status === 'PASS' && funnel.reason === 'complete') return true
  return (['M2', 'M3', 'M8', 'M9'] as const).every((metricId) => {
    const metric = input.metrics.find((item) => item.metric_id === metricId)
    return metric?.status === 'HOLD' && metric.reason === 'telemetry_incomplete'
  })
}
...
function validMetricWindowManifest(
  ...
  return (
    ...
    value.contract_version === '2026-08-09.1' &&
    value.metric_policy_version === TELEMETRY_METRIC_POLICY_VERSION &&
    value.actor_key_version === protectedActorKeyVersion() &&
    (value.cohort_role === 'baseline' || value.cohort_role === 'evaluation') &&
    value.window_start_utc === windowStartUtc &&
    value.window_end_utc === windowEndUtc &&
    ...
    (value.cohort_role === 'baseline'
      ? value.target_decisions.length === 0
  ...
  return value.metrics.every((metric) => {
    ...
    return requiresDistinct
      ? Number.isSafeInteger(metric.distinct_profiles) &&
          ...
          metric.distinct_profiles <= metric.eligible &&
...
function validMetricAggregateReceipt(input: {
  value: unknown
  source_sha: string
  generated_at_utc: string
  actor_key_version: string
  ...
  if (!isRecord(input.value)) return false
  const receipt = input.value as unknown as TelemetryMetricAggregateReceipt
  const configuredVersion = process.env.TELEMETRY_METRIC_AGGREGATE_KEY_VERSION
  const configuredKey = process.env.TELEMETRY_METRIC_AGGREGATE_COMMITMENT_KEY
  if (
    !hasExactKeys(receipt as unknown as Record<string, unknown>, [
      ...
      'metric_id',
      'required_operations',
      'operation_set_commitment',
      'cohort_commitment',
      'authority_commitment',
      'universe_commitment',
      'database_truth_commitment',
      'memory_truth_commitment',
      'eligible',
      'distinct_profiles',
      'distinct_eligible_units',
      'succeeded',
    ...
    receipt.schema_version !== TELEMETRY_METRIC_AGGREGATE_RECEIPT_SCHEMA_VERSION ||
    receipt.query_version !== TELEMETRY_QUERY_VERSION ||
    receipt.source_sha !== input.source_sha ||
    receipt.generated_at_utc !== input.generated_at_utc ||
    receipt.actor_key_version !== input.actor_key_version ||
    receipt.cohort_role !== input.cohort_role ||
    receipt.window_start_utc !== input.window_start_utc ||
    receipt.window_end_utc !== input.window_end_utc ||
    !includes(TELEMETRY_AGGREGATE_METRIC_IDS, receipt.metric_id) ||
    !Array.isArray(receipt.required_operations) ||
    !hasExactRequiredFunnelOperationSet(receipt.required_operations) ||
    receipt.authority_commitment !== input.authority_commitment ||
    receipt.universe_commitment !== input.universe_commitment ||
    receipt.receipt_key_version !== configuredVersion ||
    typeof configuredKey !== 'string' ||
    Buffer.byteLength(configuredKey, 'utf8') < TELEMETRY_COMMITMENT_KEY_MIN_LENGTH ||
    !ACTOR_KEY_VERSION_PATTERN.test(receipt.receipt_key_version) ||
    !SHA256_PATTERN.test(receipt.operation_set_commitment) ||
    !SHA256_PATTERN.test(receipt.cohort_commitment) ||
    !SHA256_PATTERN.test(receipt.authority_commitment) ||
    !SHA256_PATTERN.test(receipt.universe_commitment) ||
    !SHA256_PATTERN.test(receipt.database_truth_commitment) ||
    !SHA256_PATTERN.test(receipt.receipt_commitment) ||
    !Number.isSafeInteger(receipt.eligible) ||
    !Number.isSafeInteger(receipt.succeeded) ||
    !Number.isSafeInteger(receipt.censored) ||
    receipt.eligible < 0 ||
    receipt.succeeded < 0 ||
    receipt.censored < 0 ||
    receipt.succeeded + receipt.censored > receipt.eligible
  ) {
    return false
  }
  const requiresDistinct = ['M2', 'M3', 'M7'].includes(receipt.metric_id)
  if (
    (requiresDistinct &&
      (!Number.isSafeInteger(receipt.distinct_profiles) ||
        typeof receipt.distinct_profiles !== 'number' ||
        receipt.distinct_profiles < 0 ||
        receipt.distinct_profiles > receipt.eligible ||
        receipt.distinct_eligible_units !== receipt.eligible)) ||
    (!requiresDistinct &&
      (receipt.distinct_profiles !== null || receipt.distinct_eligible_units !== null))
  ) {
    return false
  }
  const requiresMemoryTruth = ['M2', 'M3', 'M8', 'M9'].includes(receipt.metric_id)
  if (
    (requiresMemoryTruth &&
      (typeof receipt.memory_truth_commitment !== 'string' ||
        !SHA256_PATTERN.test(receipt.memory_truth_commitment))) ||
    (!requiresMemoryTruth && receipt.memory_truth_commitment !== null)
  ) {
    return false
  }
  try {
    const expectedOperationSet = createTelemetryMetricOperationSetCommitment({
      query_version: TELEMETRY_QUERY_VERSION,
      source_sha: input.source_sha,
      actor_key_version: input.actor_key_version,
      cohort_role: input.cohort_role,
      window_start_utc: input.window_start_utc,
      window_end_utc: input.window_end_utc,
      required_operations: TELEMETRY_REQUIRED_FUNNEL_OPERATIONS,
      authority_commitment: input.authority_commitment,
      universe_commitment: input.universe_commitment,
      commitment_key: configuredKey,
    })
    if (receipt.operation_set_commitment !== expectedOperationSet) return false
    const { receipt_commitment: _commitment, ...unsignedReceipt } =
      receipt as TelemetryMetricAggregateReceipt
    const expected = Buffer.from(
      createTelemetryMetricAggregateReceiptCommitment({
        receipt: unsignedReceipt,
        commitment_key: configuredKey,
      }),
      'hex',
...
function validatedMetricAggregateTruth(input: {
  receipts: readonly TelemetryMetricAggregateReceipt[]
  source_sha: string
  generated_at_utc: string
  actor_key_version: string
  cohort_role: 'baseline' | 'evaluation'
  ...
  receipt_commitments: readonly string[]
} | null {
  if (
    !Array.isArray(input.receipts) ||
    input.receipts.length !== TELEMETRY_AGGREGATE_METRIC_IDS.length ||
    new Set(input.receipts.map((receipt) => receipt?.metric_id)).size !== input.receipts.length ||
    TELEMETRY_AGGREGATE_METRIC_IDS.some(
      (metricId) => !input.receipts.some((receipt) => receipt?.metric_id === metricId),
    ) ||
    input.receipts.some(
      (value) =>
        !validMetricAggregateReceipt({
          value,
          source_sha: input.source_sha,
    ...
    new Set(input.receipts.map((receipt) => receipt.cohort_commitment)).size !== 1 ||
    new Set(input.receipts.map((receipt) => receipt.operation_set_commitment)).size !== 1
  ) {
    return null
  }
  const byMetric = new Map(input.receipts.map((receipt) => [receipt.metric_id, receipt]))
  const eligible_census: TelemetryEligibleCensus = {
    ...
    metrics: TELEMETRY_REQUIRED_METRIC_IDS.map((metricId) => {
      const receipt = byMetric.get(metricId as (typeof TELEMETRY_AGGREGATE_METRIC_IDS)[number])
      return {
        metric_id: metricId,
        eligible: receipt?.eligible ?? 0,
        distinct_profiles: receipt?.distinct_profiles ?? null,
        distinct_eligible_units: receipt?.distinct_eligible_units ?? null,
      }
    }),
  }
  const censoring_status: TelemetryCensoringStatus = {
    ...
    metrics: TELEMETRY_REQUIRED_METRIC_IDS.map((metricId) => {
      ...
      return {
        metric_id: metricId,
        succeeded: receipt?.succeeded ?? 0,
        censored: receipt?.censored ?? 0,
      }
    }),
  }
  if (
    !validEligibleCensus(eligible_census, input.window_start_utc, input.window_end_utc) ||
  ...
  return {
    eligible_census,
    censoring_status,
    receipt_commitments: TELEMETRY_AGGREGATE_METRIC_IDS.map(
      (metricId) => byMetric.get(metricId)!.receipt_commitment,
    ),
  }
}
...
function evidenceRateResultsMatch(input: {
  ...
  return EVIDENCE_RATE_METRIC_IDS.every((metricId) => {
    ...
    if (
      input.metric_window_manifest.cohort_role === 'baseline' &&
      (metricId === 'M8' || metricId === 'M9')
    ) {
      return supplied.status === 'HOLD' && supplied.reason === 'baseline_only'
    }
...
export type TelemetryEvidence = {
  ...
  window_start_utc: string
  window_end_utc: string
  metric_aggregate_set_commitment: string
  eligible_census_commitment: string
  censoring_policy_version: 'right-censor-worst-case/v1'
  censoring_status_commitment: string
  completeness: readonly TelemetryEvidenceCompletenessResult[]
  metrics: readonly TelemetryMetricResult[]
  status: MetricStatus
  evidence_digest: string
}
...
export function buildTelemetryEvidence(input: {
  ...
  window_end_utc: string
  metric_window_manifest: TelemetryMetricWindowManifest
  metric_aggregate_receipts: readonly TelemetryMetricAggregateReceipt[]
  completeness_inputs: readonly TelemetryCompletenessInput[]
  metrics: readonly TelemetryMetricResult[]
}): TelemetryEvidence {
  const evidenceKeyVersion = process.env.TELEMETRY_EVIDENCE_KEY_VERSION
  const evidenceKey = process.env.TELEMETRY_EVIDENCE_COMMITMENT_KEY
  const actorKeyVersion = protectedActorKeyVersion()
  const sourceSha = protectedDeploymentSha()
  const generatedAtUtc = signerGeneratedAtUtc()
  const funnelInput = Array.isArray(input.completeness_inputs)
    ? input.completeness_inputs.find((item) => item?.source === 'funnel')
    : undefined
  const cohortRole = isRecord(input.metric_window_manifest)
    ? input.metric_window_manifest.cohort_role
    : null
  const aggregateTruth =
    sourceSha !== null &&
    actorKeyVersion !== null &&
    (cohortRole === 'baseline' || cohortRole === 'evaluation') &&
    funnelInput !== undefined &&
    isRecord(funnelInput.manifest) &&
    typeof funnelInput.manifest.authority_commitment === 'string' &&
    typeof funnelInput.manifest.universe_commitment === 'string'
      ? validatedMetricAggregateTruth({
          receipts: input.metric_aggregate_receipts,
          ...
          window_start_utc: input.window_start_utc,
          window_end_utc: input.window_end_utc,
          authority_commitment: funnelInput.manifest.authority_commitment,
          universe_commitment: funnelInput.manifest.universe_commitment,
        })
      : null
  if (
    !isRecord(input) ||
    ...
    aggregateTruth === null ||
    !Array.isArray(input.completeness_inputs) ||
    !Array.isArray(input.metric_aggregate_receipts) ||
    !Array.isArray(input.metrics) ||
    input.completeness_inputs.length !== SOURCES.length ||
    input.metrics.length !== TELEMETRY_REQUIRED_METRIC_IDS.length ||
    input.completeness_inputs.some(
      (item) => !validEvidenceCompletenessInput(item, input.window_start_utc, input.window_end_utc),
    ) ||
    new Set(input.completeness_inputs.map((item) => item.source)).size !==
      input.completeness_inputs.length ||
    SOURCES.some((source) => !input.completeness_inputs.some((item) => item.source === source)) ||
    ...
    !evidenceRateResultsMatch({
      window_start_utc: input.window_start_utc,
      window_end_utc: input.window_end_utc,
      generated_at_utc: generatedAtUtc,
      metric_window_manifest: input.metric_window_manifest,
      eligible_census: aggregateTruth?.eligible_census as TelemetryEligibleCensus,
      censoring_status: aggregateTruth?.censoring_status as TelemetryCensoringStatus,
      metrics: input.metrics,
      completeness: input.completeness_inputs.map(evaluateTelemetryCompleteness),
    })
  ) {
    throw new TelemetryContractError('invalid_input')
  }
  const completeness = input.completeness_inputs.map(evaluateTelemetryCompleteness)
  const status = deriveEvidenceStatus({ completeness, metrics: input.metrics })
  ...
  const orderedCompleteness = SOURCES.map((source) => {
    const completenessInput = input.completeness_inputs.find(
      (candidate) => candidate.source === source,
    )!
    const item = completeness.find((candidate) => candidate.source === source)!
    return {
      source,
      status: item.status,
      reason: item.reason,
      ...
      query_version: TELEMETRY_QUERY_VERSION,
      actor_key_version: actorKeyVersion,
      window_start_utc: completenessInput.window_start_utc,
      window_end_utc: completenessInput.window_end_utc,
      cohort_scope: completenessInput.authority_registration.cohort_rule,
      scope_commitment: createTelemetryCommitment({
        ...commitmentBoundary,
        domain: 'completeness_scope',
        value: {
          source,
          query_version: TELEMETRY_QUERY_VERSION,
          actor_key_version: actorKeyVersion,
          window_start_utc: completenessInput.window_start_utc,
          window_end_utc: completenessInput.window_end_utc,
          cohort_rule: completenessInput.authority_registration.cohort_rule,
          expected_actor: completenessInput.authority_registration.expected_actor,
          authority_commitment: completenessInput.manifest.authority_commitment,
          universe_commitment: completenessInput.manifest.universe_commitment,
        },
      }),
    }
  })
  ...
  const evidenceWithoutDigest = {
    ...
    actor_key_version: actorKeyVersion,
    cohort_role: input.metric_window_manifest.cohort_role,
    generated_at_utc: generatedAtUtc,
    ...
    window_start_utc: input.window_start_utc,
    window_end_utc: input.window_end_utc,
    metric_aggregate_set_commitment: createTelemetryCommitment({
      ...commitmentBoundary,
      domain: 'metric_aggregate_receipts',
      value: aggregateTruth.receipt_commitments,
    }),
    eligible_census_commitment: createTelemetryCommitment({
      ...commitmentBoundary,
      domain: 'eligible_census',
      value: aggregateTruth.eligible_census,
    }),
    ...
    censoring_status_commitment: createTelemetryCommitment({
      ...
      value: aggregateTruth.censoring_status,
    }),
...
export function verifyTelemetryEvidence(value: unknown): value is TelemetryEvidence {
  ...
  if (
    ...
    !hasExactKeys(value, [
      ...
      'window_start_utc',
      'window_end_utc',
      'metric_aggregate_set_commitment',
    ...
    typeof value.metric_aggregate_set_commitment !== 'string' ||
    ...
    !SHA256_PATTERN.test(value.metric_aggregate_set_commitment) ||
    ...
    new Set(value.completeness.map((item) => (isRecord(item) ? item.source : undefined))).size !==
      SOURCES.length ||
    !hasExactRequiredMetricSet(
      value.metrics.map((item) => (isRecord(item) ? item.metric_id : undefined)),
    ) ||
    value.completeness.some((item) => !validEvidenceCompletenessResult(item)) ||
    value.completeness.some(
      (item) =>
        !isRecord(item) ||
        item.window_start_utc !== value.window_start_utc ||
        item.window_end_utc !== value.window_end_utc ||
        item.actor_key_version !== value.actor_key_version,
    ) ||
    !value.completeness.some(
      (item) => isRecord(item) && item.source === 'funnel' && item.cohort_scope === 'all_actors',
    ) ||
    value.metrics.some((item) => !validMetricResult(item as never)) ||
    !includes(METRIC_STATUSES, value.status) ||
    value.status !==
      deriveEvidenceStatus({
        completeness: value.completeness as unknown as TelemetryEvidenceCompletenessResult[],
        metrics: value.metrics as unknown as TelemetryMetricResult[],
      }) ||
    !validEvidenceCompletenessPropagation({
      completeness: value.completeness as unknown as TelemetryEvidenceCompletenessResult[],
      metrics: value.metrics as unknown as TelemetryMetricResult[],
    }) ||
    (value.cohort_role === 'baseline' &&
      !(['M8', 'M9'] as const).every((metricId) => {
        const metric = (value.metrics as unknown as TelemetryMetricResult[]).find(
          (item) => item.metric_id === metricId,
        )
        return metric?.status === 'HOLD' && metric.reason === 'baseline_only'
      })) ||
    (value.cohort_role === 'evaluation' &&
      (value.metrics as unknown as TelemetryMetricResult[]).some(
        (metric) => metric.reason === 'baseline_only',
      ))
  ) {
    return false
  }
